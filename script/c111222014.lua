...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
    ...
    while tc do
        Debug.Message("Inspecting card: " .. tc:GetCode())
        ...
        if tc:IsControler(tp) 
            ...
            and rp==1-tp then
            Debug.Message("Condition met for card: " .. tc:GetCode())
    ...
    Debug.Message("Condition not met.")
    return false
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
    Debug.Message("Activating 'My Brother's' effect...")
    local mat=e:GetLabelObject()
    ...
    for tc in aux.Next(mat) do
        Debug.Message("Checking material: " .. tc:GetCode())
        -- Search in Hand
        local hg=Duel.GetMatchingGroup(function(c) return c:GetCode()==tc:GetCode() and s.spfilter(c,e,tp) end,tp,LOCATION_HAND,0,nil)
